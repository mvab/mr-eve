...
parser$add_argument('--headerid', required=TRUE)
...
message(id)
mrpath <- file.path(args[["outdir"]], "data", id, "mr.rdata")
stopifnot(file.exists(mrpath))
hetpath <- file.path(args[["outdir"]], "data", id, "heterogeneity.rdata")
...
dir.create(file.path(args[["outdir"]], "data", id, "neo4j_stage"))
mrout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_mr.csv.gz"))
hetout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_het.csv.gz"))
moeout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_moe.csv.gz"))
intout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_int.csv.gz"))
metout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_met.csv.gz"))
vtout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_vt.csv.gz"))
instout <- file.path(args[["outdir"]], "data", id, "neo4j_stage", paste0(id, "_inst.csv.gz"))
...
vtpath <- file.path(args[["outdir"]], "data", id, "ml.csv.gz")
stopifnot(file.exists(vtpath))

a <- readml(vtpath, idinfo, format="none")
a <- dplyr::filter(a, SNP %in% rsid)
a$ogid <- id
...
fn <- function(nom)
{
  file.path(args[["outdir"]], "outdir",  "resources", "neo4j_stage", paste0("header_", nom, ".csv.gz"))
}

dir.create(file.path(args[["outdir"]], "outdir", "resources", "neo4j_stage"))
